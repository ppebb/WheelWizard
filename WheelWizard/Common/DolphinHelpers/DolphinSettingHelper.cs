...
namespace CT_MKWII.Common.DolphinHelpers;
...
public class DolphinSettingHelper
{
    public static string ReadINISetting(string FileLocation, string Section, string SettingToRead)
    {
        ...
        {
            UIProvider.DialogBox.Show("Something went wrong, INI file could not be read, Message Patchzy with the following error: " + FileLocation, "Error", DialogBoxButtons.OK, DialogBoxIcon.Error);
        }
        ...
        if (!sectionFound)
        {
            // UIProvider.DialogBox.Show("Could not find section in INI file, Message Patchzy with the following error: " + Section, "Error", DialogBoxButtons.OK, DialogBoxIcon.Error);
            return "";
        }
        ...
        // UIProvider.DialogBox.Show("Could not find setting in INI file, Message Patchzy with the following error: " + SettingToRead, "Error", DialogBoxButtons.OK, DialogBoxIcon.Error);
        return "";
    }
    ...
    public static void ChangeINISettings(string fileLocation, string section, string settingToChange, string value)
    {
        if (!File.Exists(fileLocation))
        {
            UIProvider.DialogBox.Show($"Something went wrong, INI file could not be found, Message Patchzy with the following error: {fileLocation}", "Error", DialogBoxButtons.OK, DialogBoxIcon.Error);
            return;
        }
    ...
    public static void LaunchDolphin(string arguments = "")
    {
        ...
        if (DolphinLocation == "")
        {
            UIProvider.DialogBox.Show("Could not find Dolphin Emulator, please set the path in settings", "Error", DialogBoxButtons.OK, DialogBoxIcon.Error);
            return;
        }
