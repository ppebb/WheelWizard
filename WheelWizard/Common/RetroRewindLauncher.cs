...
public static class RetroRewindLauncher
{
    public static void PlayRetroRewind(bool playTT)
    {
        ...
        {
            UIProvider.DialogBox.Show("Message patchzy with the following error: " + dolphinLocation + " " + gamePath, "Error", DialogBoxButtons.OK,
                DialogBoxIcon.Error);
            return;
        }
        try
        ...
        {
            Console.WriteLine(e);
            UIProvider.DialogBox.Show("Failed to start Dolphin Emulator. Please try again.", "Error", DialogBoxButtons.OK,
                DialogBoxIcon.Error);
        }
    }
    ...
    private static void GenerateLaunchJSON(bool launchTT)
    {
        ...
        // TODO: Why not just use interpolation???? Tweaker shit
