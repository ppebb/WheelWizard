...
public static class RetroRewindInstaller
{
    ...
    public static async Task<string> GetLatestVersionString()
    {
        ...
        {
            UIProvider.DialogBox.Show($"Failed to check for updates: {ex.Message}");
    ...
    public static async Task<bool> UpdateRR()
    {
        ...
        {
            UIProvider.DialogBox.Show("Retro Rewind is up to date.");
            return true;
        }
        ...
        if (currentVersion == "Not Installed")
        {
            ...
            var result = UIProvider.DialogBox.Show("Your version of Retro Rewind could not be determined. Therefore, we cannot update. Would you like to download Retro Rewind?", "Download Retro Rewind", DialogBoxButtons.YesNo);
        ...
        {
            var result = UIProvider.DialogBox.Show("Your version of Retro Rewind is too old to update. Would you like to reinstall Retro Rewind?", "Reinstall Retro Rewind", DialogBoxButtons.YesNo);
            if (result == DialogBoxResponse.No) return false;
            await InstallRetroRewind();
            return true;
        }
        ...
        foreach (var update in updatesToApply)
        {
            ...
            if (!success)
            {
                UIProvider.DialogBox.Show("Failed to apply an update. Aborting.");
    ...
    private static async Task<bool> DownloadAndApplyUpdate((string Version, string Url, string Path, string Description) update, int totalUpdates, int currentUpdateIndex, IProgressWindow window)
    {
        var loadPath = SettingsUtils.GetLoadPathLocation();
        ...
        try
        {
            ...
            window.SetProgress(100, "Extracting update...");
        ...
        {
            UIProvider.DialogBox.Show($"An error occurred while applying the update: {ex.Message}");
            return false;
        }
        finally
        {
    ...
    public static async Task InstallRetroRewind()
    {
        ...
        {
            var result = UIProvider.DialogBox.Show("There are already files in your RetroRewind Folder. Would you like to install?", "Reinstall Retro Rewind", DialogBoxButtons.YesNo);
            if (result == DialogBoxResponse.No)
            {
                return;
            }
        ...
        IProgressWindow progressWindow = UIProvider.ProgressWindowFactory.NewProgressWindow();
        progressWindow.Show();
